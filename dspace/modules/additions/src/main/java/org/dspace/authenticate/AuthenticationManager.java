...
import org.dspace.authenticate.SSO;
import org.dspace.eperson.EPerson;
import org.dspace.core.Context;
...
public class AuthenticationManager
{
    ...
    private static int authenticateInternal(Context context,
                                            ...
                                            boolean implicitOnly) throws SQLException
    {
        ...
        for (int i = 0; i < methodStack.length; ++i)
        {
            if (!implicitOnly || methodStack[i].isImplicit())
            {
                ...
                {
                    if (!"".equals(SSO.user_check(username,password)))
                    {
                        EPerson eperson = null;
                        Context usr = new Context();
                        try {
                            eperson.create(usr);
                            ...
                            log.error("update user data error", ex);
                        }
                        ret = AuthenticationMethod.SUCCESS;
                    }
                    else
                    {
